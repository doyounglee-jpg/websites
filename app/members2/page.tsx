...
/**
 * Clerkie /members2 — Horizon Hero variant of /members.
 *
 * Same B&W Linear-style language as /members but with atmospheric
 * weight: a stipple-art horizon image as the hero, a dotted horizon
 * divider between sections, and a starfield texture behind a few key
 * surfaces. Info structure is identical to /members.
 *
    ...
    <main className="min-h-screen bg-[#08090A] text-zinc-50">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-white/[0.06] bg-[#08090A]/60 backdrop-blur-md">
        <nav className="mx-auto flex w-full max-w-[1440px] items-center justify-between px-6 py-4 sm:px-10 sm:py-5 md:px-16">
          ...
          {/* Desktop: Log in + Sign up. Hidden on mobile — hamburger drawer takes over. */}
      ...
      {/* ============================================================
          2. HERO — horizon image as backdrop, headline floats in front
         ============================================================ */}
      <section className="relative min-h-[760px] overflow-hidden sm:min-h-[860px] lg:min-h-[940px]">
        {/* Stipple horizon — full-bleed, bottom-aligned. The sun sits
            roughly center-vertical of the image, so we crop the image
            with object-bottom so the horizon line stays at the bottom
            of the section. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/horizon.png"
          alt=""
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-[88%] w-full select-none object-cover object-bottom"
        />
        {/* Top fade — blends image into page background so there's no
            hard top edge against the nav. */}
        <div
          className="pointer-events-none absolute inset-x-0 top-0 z-[1] h-[45%] bg-gradient-to-b from-[#08090A] via-[#08090A]/60 to-transparent"
          aria-hidden="true"
        />
        {/* Side fade — softens the image edges left/right so it feels
            like an endless horizon rather than a framed picture. */}
        <div
          className="pointer-events-none absolute inset-y-0 left-0 z-[1] w-[20%] bg-gradient-to-r from-[#08090A] to-transparent"
          aria-hidden="true"
        />
        <div
          className="pointer-events-none absolute inset-y-0 right-0 z-[1] w-[20%] bg-gradient-to-l from-[#08090A] to-transparent"
          aria-hidden="true"
        />
        {/* Center vignette — softens the sun's brightness behind the text
            block so the headline + subhead stay readable. Subtle: just
            enough contrast for type without dimming the sun's drama. */}
        <div
          className="pointer-events-none absolute left-1/2 top-1/2 z-[2] h-[600px] w-[900px] -translate-x-1/2 -translate-y-1/2"
          style={{
            background:
              "radial-gradient(ellipse 50% 60% at 50% 50%, rgba(8,9,10,0.55) 0%, rgba(8,9,10,0.25) 40%, transparent 75%)",
          }}
          aria-hidden="true"
        />

        {/* Foreground content — eyebrow, headline, subhead, CTAs */}
        <div className="relative z-10 mx-auto flex min-h-[760px] max-w-[1440px] flex-col items-center justify-center gap-8 px-6 pb-32 pt-16 sm:min-h-[860px] sm:gap-10 sm:px-10 sm:pb-40 sm:pt-24 md:px-16 md:pt-28 lg:min-h-[940px]">
          {/* Eyebrow pill */}
          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-[#08090A]/60 py-1.5 pl-2 pr-3.5 backdrop-blur-sm">
          ...
          {/* Headline + subhead */}
          <div className="flex max-w-[920px] flex-col items-center gap-6 sm:gap-7">
            <h1
              className="text-center text-[44px] font-semibold leading-[1.05] tracking-[-0.045em] sm:text-[60px] md:text-[80px] lg:leading-[0.98] lg:text-[104px]"
              style={{
                textShadow:
                  "0 2px 40px rgba(8,9,10,0.6), 0 1px 8px rgba(8,9,10,0.5)",
              }}
            >
              Solve your debt and money problems.
            </h1>
            <p
              className="max-w-[580px] text-center text-base leading-[1.55] tracking-[-0.005em] text-zinc-200 sm:text-lg lg:text-[19px]"
              style={{
                textShadow:
                  "0 2px 24px rgba(8,9,10,0.85), 0 0 12px rgba(8,9,10,0.6)",
              }}
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="#"
              className="rounded-[10px] border border-white/15 bg-[#08090A]/60 px-5 py-3 text-sm font-medium tracking-[-0.005em] backdrop-blur-sm"
            >
              Talk to an advisor
            </a>
          </div>
      ...
      <section>
        <div className="mx-auto max-w-[1440px] px-6 pb-20 pt-24 sm:px-10 sm:pb-24 sm:pt-28 md:px-16 md:pb-28 md:pt-32 lg:px-24 lg:pb-32 lg:pt-40">
          ...
          {/* Three columns: stat / notif stack / stat — stack on mobile */}
          <div className="relative flex flex-col items-stretch gap-6 py-8 lg:flex-row lg:gap-10 lg:py-15">
            <div
              className="aurora-mono-tight pointer-events-none absolute left-1/2 top-[30px] h-[400px] w-[900px] -translate-x-1/2"
              aria-hidden="true"
            />
            ...
            <div className="relative z-10 flex flex-[1.4] flex-col gap-3">
              <NotifHot />
              <NotifCalm
            ...
            <SafetyStatBlock label="MEMBERS HELPED" value="142,803">
              <div className="h-px bg-white/[0.06]" />
              <p className="text-[13px] leading-[1.55] tracking-[-0.005em] text-zinc-400">
                Real members covered through unexpected expenses without
                spiraling into payday debt — the average advance is $342, fully
                repaid within 17 days.
              </p>
            </SafetyStatBlock>
          </div>
        </div>
    ...
    <div className="flex flex-1 flex-col overflow-hidden rounded-[18px] border border-white/10 bg-[#101113]/60 backdrop-blur">
      ...
      <div className="flex items-center justify-between border-b border-white/[0.06] px-4.5 py-3.5">
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-md bg-zinc-50">
            <div className="h-1.5 w-1.5 rounded-full bg-[#08090A]" />
          </div>
      ...
      <div className="flex flex-col gap-4 px-6 py-6">
        ...
        <div className="flex flex-col items-start gap-2.5 pr-15">
          <div className="flex items-center gap-2">
            <div className="flex h-[22px] w-[22px] items-center justify-center rounded-md bg-zinc-50">
              <div className="h-[5px] w-[5px] rounded-full bg-[#08090A]" />
            </div>
            <span className="text-xs font-medium tracking-[0.02em] text-zinc-500">
...
function NotifCalm({
  ...
  return (
    <div className="flex items-start gap-3 rounded-[14px] border border-white/[0.06] bg-white/[0.03] px-4.5 py-4">
      <span className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-md border border-white/[0.06] bg-white/[0.08]">
